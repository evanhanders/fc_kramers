...
class FC_MHD_equations(FC_equations):
    ...
    def _set_subs(self):
        ...
        self.problem.substitutions['Bz'] = '(dx(Ay) - dy(Ax))'
        self.problem.substitutions['Jx'] = '(dy(Bz) - dz(By))'
        self.problem.substitutions['Jy'] = '(dz(Bx) - dx(Bz))'
        self.problem.substitutions['Jz'] = '(dx(By) - dy(Bz))'
        
        self.problem.substitutions['BdotGrad(f, f_z)'] = "(Bx*dx(f) + By*dy(f) + Bz*(f_z))"
    ...
    def initialize_output(self, solver, data_dir, **kwargs):
        ...
        analysis_slice.add_task("dx(Bx) + dy(By) + dz(Bz)", name="divB")
...
class FC_MHD_equations_2d(FC_MHD_equations, FC_equations_2d):
    ...
    def set_equations(self, Rayleigh, Prandtl, MagneticPrandtl,
                            Taylor=None, theta=0,
                            split_diffusivities=False, **kwargs):
        ...
        self.problem.add_equation("dz(w) - w_z = 0")
        self.problem.add_equation("dz(T1) - T1_z = 0")
        self.problem.add_equation("Bx + dz(Ay) - dy(Az) = 0")
        self.problem.add_equation("By - dz(Ax) + dx(Az) = 0")

        self.problem.add_equation(("(scale)*( dt(w) + Coriolis_z +   T1_z + T0*dz(ln_rho1) + T1*del_ln_rho0 - L_visc_w) = "
                                   "(scale)*(-T1*dz(ln_rho1) - UdotGrad(w, w_z) + R_visc_w  + 1/(4*pi*rho_full)*(Jx*By - Jy*Bx))"))

        self.problem.add_equation(("(scale)*( dt(u) + Coriolis_x + dx(T1) + T0*dx(ln_rho1)                  - L_visc_u) = "
                                   "(scale)*(-T1*dx(ln_rho1) - UdotGrad(u, u_z) + R_visc_u  + 1/(4*pi*rho_full)*(Jy*Bz - Jz*By))"))

        self.problem.add_equation(("(scale)*( dt(v) + Coriolis_y + dy(T1) + T0*dy(ln_rho1)                  - L_visc_v) = "
                                   "(scale)*(-T1*dy(ln_rho1) - UdotGrad(v, v_z) + R_visc_v  + 1/(4*pi*rho_full)*(Jz*Bx - Jx*Bz))"))
        ...
        self.problem.add_equation("dt(Ay) + eta*Jy + dy(Phi)            =  w*Bx - u*Bz")
...
class FC_MHD_equations_guidefield(FC_MHD_equations):
    ...
    def _set_parameters(self, Bz=None):
        super(FC_MHD_equations_guidefield, self)._set_parameters()
        if Bz is None:
            self.problem.substitutions['Bz_0'] = '0'
        else:
            self.problem.parameters['Bz_0'] = Bz
            logger.info("Constant Bz guidefield, amplitude {}".format(Bz))
        self.problem.substitions['Bx_0'] = '0'
        self.problem.substitions['By_0'] = '0'

        self.problem.substituons['Jx_0'] = '0'
        self.problem.substituons['Jy_0'] = '0'
        self.problem.substituons['Jz_0'] = '0'
...
class FC_equations_MHD_guidefield_2d(FC_MHD_equations_guidefield, FC_equations_2d):
    ...
    def set_equations(self, Rayleigh, Prandtl, MagneticPrandtl, guidefield_amplitude,
                      threeD = False,
                      Taylor=None, theta=0,
                      split_diffusivities=False,**kwargs):
        # DOES NOT YET INCLUDE Ohmic heating
        ...
        self.split_diffusivities=split_diffusivities
        self._set_diffusivities(Rayleigh, Prandtl, MagneticPrandtl, **kwargs)
        self._set_parameters(Bz=guidefield_amplitude)
        self._set_subs()
        ...
        self.problem.add_equation("dz(w) - w_z = 0")
        self.problem.add_equation("dz(T1) - T1_z = 0")
        self.problem.add_equation("Bx + dz(Ay) - dy(Ax) = 0")
        self.problem.add_equation("By - dz(Ax) + dx(Az) = 0")
        ...
        self.problem.add_equation(("(scale)*( dt(v) + dy(T1) + T0*dy(ln_rho1)                 - L_visc_v "
                                   ...
                                   "(scale)*(-T1*dy(ln_rho1) - UdotGrad(v, v_z) + R_visc_v  "
        ...
        self.problem.add_equation("dt(Ay) + eta*Jy + dy(Phi) - (w*Bx_0 - u*Bz_0) =  w*Bx - u*Bz - eta*Jy_0")
